...
export default ({ test, promise_test }) => {
  ...
  test(() => {
    ...
    assert_throws_exactly(
      ...
      () =>
        new ReadableStream({
          // @ts-expect-error - invalid overload
          get pull() {
            throw theError
          },
        }),
    ...
    return Promise.all([
      reader.read().then((r) => {
        assert_object_equals(
          r,
          { value: "a", done: false },
          "the first chunk read should be correct"
        )
      }),
      reader.read().then((r) => {
        assert_object_equals(
          r,
          { value: "a", done: false },
          "the second chunk read should be correct"
        )
    ...
    return Promise.all([
      reader.read().then((r) => {
        assert_object_equals(
          r,
          { value: "a", done: false },
          "the chunk read should be correct"
        )
      }),
  ...
  promise_test(() => {
    ...
    assert_throws_js(
      ...
      "Calling enqueue after canceling should throw"
    )
    ...
    return new ReadableStream({
      start(c) {
        ...
        assert_throws_js(
          TypeError,
          () => c.enqueue("a"),
          "call to enqueue should throw a TypeError"
        )
      },
    }).getReader().closed
    ...
    const closed = new ReadableStream({
      start(c) {
        ...
        assert_throws_js(
          TypeError,
          () => c.enqueue("a"),
          "call to enqueue should throw the error"
        )
      },
    }).getReader().closed
    ...
    return new ReadableStream({
      start(c) {
        ...
        assert_throws_js(
          TypeError,
          () => c.close(),
          "second call to close should throw a TypeError"
        )
      },
    }).getReader().closed
    ...
    return Promise.all([
      reader.read().then((r) => {
        assert_object_equals(
          r,
          { value: "a", done: false },
          "read() should read the enqueued chunk"
        )
        readCalled = true
      }),
      reader.closed.then(() => {
  ...
  promise_test(() => {
    ...
    assert_throws_js(
      TypeError,
      () => controller.close(),
      "Calling close after canceling should throw"
    )
    ...
    const closed = new ReadableStream({
      start(c) {
        ...
        assert_throws_js(
          TypeError,
          () => c.close(),
          "call to close should throw a TypeError"
        )
        startCalled = true
      },
    }).getReader().closed
